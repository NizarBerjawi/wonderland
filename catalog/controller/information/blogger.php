...
class ControllerInformationBlogger extends Controller {
	...
	public function index() {
		...
		$data['date_added'] = date($this->language->get('date_format'), strtotime($blog_info['date_added']));
		...
		if ($blog_comments) {
			foreach ($blog_comments as $comment) {
				$data['blog_comments'][] = array(
					...
					'date_added' => date($this->language->get('date_format'), strtotime($comment['date_added']))
				);
			}
		}
			...
			if (!empty($setting['status']) && $setting['status']) {
				...
				$total_comments = $this->model_megnor_blogger->getTotalBlogComments($module_blog['blogger_id']);
					...
					$module_blog_data[] = array(
						...
						'date_added'  => date($this->language->get('date_format'), strtotime($module_blog['date_added'])),
